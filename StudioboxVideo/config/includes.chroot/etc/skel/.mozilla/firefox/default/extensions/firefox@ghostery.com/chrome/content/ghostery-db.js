...
ghostery.db = {
	...
	refreshOnUpdate: function() {
		try {
			var embeddedVersion, currentVersion, is, sis, data,
			 file = ghostery.prefs.getProfileDirectory(),
			 embedded = Components.classes["@mozilla.org/moz/jssubscript-loader;1"].getService(Components.interfaces.mozIJSSubScriptLoader).loadSubScript("chrome://ghostery/content/ghostery-bugs.json");
			...
			currentVersion = parseInt(this.fromJSON(data).bugsVersion, 10);
			embeddedVersion = parseInt(embedded.bugsVersion, 10);
	...
	loadBugs: function() {
		...
		// set all entries in the official database non-user created
