...
ghostery = {
			...
			gBrowser.addProgressListener({
				...
				onStateChange: function(p, r, f, status) {
					if ( (f & Components.interfaces.nsIWebProgressListener.STATE_START) && 
						...
						try {
							// This clears the results array for the top page.
							// TODO: Clear all other results?
							...
							// This clears associated frame results
							delete ghostery.prefs.frameResults[r.URI.spec];
						} catch (e) {}

						// move preCensusCache to censusCache
						for (var entry in ghostery.preCensusCache) {
							ghostery.censusCache[entry] = ghostery.preCensusCache[entry];
						}
						ghostery.preCensusCache = {};
					}
				}
			});
	...
	// leaving this in for Firefox 20-
...
// TODO: sometime shows partial list?
			...
			ghostery.lastUI[page] = detected.length;
	...
	analyzePage: function(doc, countBug) {
		...
		for (var i = 0; i < html.length; i++) {
			...
			if (src) {
				...
				// img with source set to be bsae64 image/canvas.
				if (html[i].nodeName == 'IMG' && (src.indexOf('data:image') == 0)) {
					continue;
				}
					...
					for (var j = 0; j < trackers.length; j++) {
						...
						if ( (page != srcDoc) && ( tracker.policyContentLocation.toLowerCase() == src.toLowerCase() ) ) { // this is a framed tracker, add to the top level doc
							...
							if ( (!tracker.policyBlocked) ) {
								...
								// removing only if its still present. parent maybe modified in click2play
								try { html[i].parentNode.removeChild(html[i]); } catch (err) {}
							}
							break;
						}
					}
		...
		if ( ghostery.prefs.shareData && !isFrame && (ghostery.prefs.getBugs(srcDoc).length > 0) ) {
	...
	analyzePageInfo: function(doc, domain) {
		...
		if ( ( (pageLatency > 0) || (spots > 0) ) 
				&& !ghostery.prefs.isWindowPrivate(gBrowser.selectedBrowser.contentDocument) )
	...
	processWorkerResponse: function(event) {
		try {
			if (event.data.action == 'stage11') {
				...
				if (isBug) {
					this.invokeWorker({"action": "stage22", "uid": tracker.id, "src": tracker.policyContentLocationQueryless});
				} else {
					// not a bug, remove from the map.
					delete ghostery.resultsUidMap[uid];
				}
				...
				// check if we need to populate hasNewApp & update UI only if there is a new entry.
				if ( gBrowser.selectedBrowser.contentDocument.location.href == tracker.policyRequestOrigin ) {
					hasNewApp = ( ghostery.lastUI[gBrowser.selectedBrowser.contentDocument.location.href] != ghostery.prefs.countApps(ghostery.prefs.getBugs(gBrowser.selectedBrowser.contentDocument.location.href)) );
				...
				// done processing this one, remove from map.
				delete ghostery.resultsUidMap[uid];
	...
	checkC2P: function (tracker, doc) {
		...
		for (var _c2p in ghostery.prefs.click2play[bug.aid]) {
			...
			try {
				...
				// adding a check for reinjection to make sure its only done for scripts.
				if (reinject && tracker.policyContentType != 2) {
					reinject = false;
				}
	...
	recordStats: function(bugs, tracker) {
		...
		if ( bugs.bug && domain ) {
			// First check if current [domain]:[bug id] has already been submitted for today
			var domain_bug = domain + '|' + bugs.bug.id,
			 now = new Date(),
			...
			if ( domain_bug in ghostery.censusCache) {
				...
				//ghostery.censusCache[domain_bug] = today;
				ghostery.preCensusCache[domain_bug] = today;
			}
