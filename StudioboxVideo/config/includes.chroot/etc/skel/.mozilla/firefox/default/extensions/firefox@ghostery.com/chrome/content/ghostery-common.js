 ...
 */
	...
	logCP: function(cp) {
		...
		if (!contains(existing, cp.policyContentLocation)) {
			...
			q = cp.policyContentLocation.indexOf('?');

			// strip out the querystring, including the ?, to reduce false positives
			if (q >= 0) {
				cp.policyContentLocationQueryless = cp.policyContentLocation.slice(0, q);
			} else {
				cp.policyContentLocationQueryless = cp.policyContentLocation;
			}

			ghostery.invokeWorker({"action": stage, "uid": cp.id, "src": cp.policyContentLocationQueryless});
